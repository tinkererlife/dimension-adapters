...
import { createRetentionAdapter, RetentionActivity, RetentionConfig, RetentionQueryOptions } from "../../helpers/retention";
...
async function getActivity(options: RetentionQueryOptions, fromDay: string, toDayExclusive: string): Promise<RetentionActivity[]> {
  const operators = await options.api.call({ target: REGISTRY, abi: abis.listTrustedOperatorRoleMembers });
  const forwarders = await options.api.call({ target: REGISTRY, abi: abis.listTrustedForwarderRoleMembers });
  const minterMembers = await options.api.call({ target: REGISTRY, abi: abis.listMinterRoleMembers });
  ...
  const rows: DuneActivityRow[] = await queryDuneSql(options, `
...
FROM (
  ...
  FROM polygon.logs l
  WHERE l.block_date >= date '${fromDay}' AND l.block_date < date '${toDayExclusive}'
...
export const retentionConfig: RetentionConfig = {
  ...
  // ~712k wallets and growing ~25k/week: sharded so no run holds the full
  // first-seen table in memory.
  shardCount: 32,
...
export default createRetentionAdapter(retentionConfig);
